...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Owner: {owner.email} (ID: {owner.id}, Role: {owner.role})"))
        except User.DoesNotExist:
            self.stdout.write(self.style.WARNING("Пользователь с ID 7 не найден! Создаём нового."))
            ...
            self.stdout.write(self.style.SUCCESS(f"Создан новый owner: {owner.email} (ID: {owner.id}, Role: {owner.role})"))

        # Гурман (берём ID 8 или создаём нового)
        try:
            gourmand = User.objects.get(id=8)
            self.stdout.write(self.style.SUCCESS(f"Gourmand: {gourmand.email} (ID: {gourmand.id}, Role: {gourmand.role})"))
        except User.DoesNotExist:
            self.stdout.write(self.style.WARNING("Гурман с ID 8 не найден! Создаём нового."))
            ...
            self.stdout.write(self.style.SUCCESS(f"Создан новый gourmand: {gourmand.email} (ID: {gourmand.id}, Role: {gourmand.role})"))
        ...
        for place in places:
            for _ in range(random.randint(2, 10)):  # 8-16 отзывов на заведение
                ...
                review = Review.objects.create(
                    name=f"Отзыв {review_counter}",  # Уникальное название
        ...
        # Проверка
        self.stdout.write(self.style.SUCCESS(f"Places for owner {owner.email}: {Place.objects.filter(owner=owner).count()}"))
        self.stdout.write(self.style.SUCCESS(f"Reviews by gourmand {gourmand.email}: {Review.objects.filter(gourmand=gourmand).count()}"))
        self.stdout.write(self.style.SUCCESS(f"NPS Responses: {NPSResponse.objects.count()}"))